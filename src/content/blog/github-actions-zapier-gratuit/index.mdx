...
Combinés à des [**cron jobs**](/blog/cron-job-github-actions), les GitHub Actions deviennent très puissants pour exécuter n’importe quel type de script régulièrement, comme générer des rapports, créer des codes promos, ou envoyer des notifications.

### 2. Cron job : programmer tes automatisations

Un [**cron jobs**](/blog/cron-job-github-actions), c'est tout simplement un moyen de programmer automatiquement le lancement d'une tâche à intervalles réguliers sur un serveur. Tu peux dire à GitHub : "exécute ce script chaque jour à minuit", et GitHub s’en occupe automatiquement.

Si tu veux savoir exactement comment programmer un cron job dans GitHub Actions, je t’ai préparé un tutoriel facile à suivre ici : [Comment utiliser un cron job avec GitHub Actions](/blog/cron-job-github-actions).
